...
int closesplash(splash* p)
{
    if (!p)
        return -1;
    if (p->sig != libmshlsplash_signature)
        return -1;
    fclose(p->f);
    free(p->compbuf);
    free(p->decompbuf);
...
splash* opensplash(const char* path)
{
    ...
    if (fread(&p->flags, 1, 4, p->f) < 4 || fread(&p->frame, 1, 4, p->f) < 4 || p->frame > 999 ||
        !(p->head = (splash_frame*)malloc(16 * p->frame)))
    {
        fclose(p->f);
        free(p->compbuf);
        free(p->decompbuf);
    ...
    for (i = 0; i < p->frame; i++)
    {
        ...
        if (p->head[i].size1 > 0xc000)
            flag1 = 1;
        if (p->head[i].size2 > 0x18000)
            flag2 = 1;
        size1total += size1;
        size2total += size2;
    }
...
int decompressimage(splash* p, int idx)
{
    ...
    switch (p->version)
    {
        ...
        case SPLASH_ISAKUREAL:
        {
            ...
            if (iSakuRealDecode(&mcomp, &mdecomp, &mdecompbuf, palcnt, p->compbuf + 2))
                return 1;
            return 0;
        }
        case SPLASH_MOONSHELL2:
        {
            ...
            if (idx > 0)
            {
                ...
                {
                    if (!(p->compbuf[i] & 0x8000))
                        dest += p->compbuf[i];
                    else
                        p->decompbuf[dest] = p->compbuf[i], dest++;
                }
            }
            return 0;
        }
