...
int lzmaOpen7z()
{
    ...
    if (!h7z)
        h7z = LoadLibraryA("/usr/local/lib/p7zip/7z.so");
    if (!h7z)
        h7z = LoadLibraryA("/opt/local/lib/p7zip/7z.so"); // MacPorts
    if (!h7z)
        h7z = LoadLibraryA("/sw/lib/p7zip/7z.so"); // Fink
    if (!h7z)
        h7z = LoadLibraryA("7z.so");
    if (!h7z)
        h7z = LoadLibraryA("7z");
#endif
#endif
...
int lzmaCreateArchiver(void** archiver, unsigned char arctype, int encode, int level)
{
    ...
    if (pCreateArchiver(&CLSID_CArchiveHandler, encode ? &IID_IOutArchive : &IID_IInArchive, archiver) != S_OK)
    {
        *archiver = NULL;
        return 1;
    }
...
int lzmaDestroyArchiver(void** archiver, int encode)
{
    if (!h7z || !archiver)
        return -1;
    if (encode)
        (*(IOutArchive**)archiver)->Release();
    else
        (*(IInArchive**)archiver)->Release();
    *archiver = NULL;
    return 0;
}
#if 0
...
int lzmaCreateCoder(void** coder, unsigned long long int id, int encode, int level)
{
    ...
    if (pCreateCoder(&CLSID_CCodec, &IID_ICompressCoder, coder) != S_OK)
    {
        *coder = NULL;
        return 1;
    }
    ...
    if (id == 0x040108 || id == 0x040109)
    {
        ...
        {
            case 9:
                VAR[0].ulVal = 10, VAR[1].ulVal = 255;
                break;
            case 8:
                VAR[0].ulVal = 8, VAR[1].ulVal = 255;
                break;
            case 7:
                VAR[0].ulVal = 6, VAR[1].ulVal = 128;
                break;
            case 6:
                VAR[0].ulVal = 5, VAR[1].ulVal = 128;
                break;
            case 5:
                VAR[0].ulVal = 4, VAR[1].ulVal = 128;
                break;
            case 4:
                VAR[0].ulVal = 3, VAR[1].ulVal = 64;
                break;
            case 3:
                VAR[0].ulVal = 2, VAR[1].ulVal = 64;
                break;
            case 2:
                VAR[0].ulVal = 2, VAR[1].ulVal = 32;
                break;
            case 1:
                VAR[0].ulVal = 1, VAR[1].ulVal = 32;
                break;
        }
...
static int mread2(void* h, char* p, int n)
{
    return mread(p, n, (memstream*)h);
}
static int mwrite2(void* h, const char* p, int n)
{
    return mwrite(p, n, (memstream*)h);
}
static int mclose2(void* h)
{
    return 0;
}
int lzmaCodeOneshot(void* coder, unsigned char* in, size_t isize, unsigned char* out, size_t* osize)
{
    if (!h7z || !in || !out || !osize || !*osize)
        return -1;
