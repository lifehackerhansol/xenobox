...
static unsigned int textsize,               /* text size counter */
...
may be spuriously long near the end of text. */
        ...
        for (i = 0; i < last_match_length && (c = mgetc(infile)) != EOF; i++)
        {
    ...
    string comparison easier. */
        ...
        {                  /* After the end of text, */
            ...
            r = (r + 1) & (N - 1);
            if (--len)
                InsertNode(r, VRAMSafe); /* buffer may not be empty. */
        }
...
// also fixed to decode in the way bios funktion works
{
    ...
    for (;;)
    {
        ...
        {
            if ((i = mgetc(infile)) == EOF)
                break;
            if ((j = mgetc(infile)) == EOF)
                break;
