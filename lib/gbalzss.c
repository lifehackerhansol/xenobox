/*
	gbalzss.c - filter for NDS swiDecompressLZ77[WV]ram format
	http://devkitpro.svn.sourceforge.net/viewvc/devkitpro/trunk/tools/gba/gbatools/

	X: removed
	for (i = 1; i <= F; i++) InsertNode(r - i, VRAMSafe);
	according to wingrit (not copied...)
*/
...
// function patched to produce gba bios decompression function processable data
{
	...
	u32 gbaheader = 0x10 + (mlength(infile)<<8);
	unsigned char* tmp = (unsigned char*)&gbaheader;
	//printf("gba header: %x\n", gbaheader );
	for(i=0; i<4; i++) mputc( tmp[i], outfile );
	...
	code_buf_ptr = 1;
	mask = 0x80;	// f・ GBA fangen wir mit MSB an
	s = 0;
	r = N - F; 	// 4078

	for (i = s; i < r; i++) text_buf[i] = 0xff;  /* Clear the buffer with any character that will appear often. */
	for (len = 0; len < F && (c = mgetc(infile)) != EOF; len++)
		text_buf[r + len] = c;  /* Read F bytes into the last F bytes of
			the buffer */
	if ((textsize = len) == 0) return;  /* text of size zero */

	//for (i = 1; i <= F; i++) InsertNode(r - i, VRAMSafe); /*** fixed by X ***/
  /* Insert the F strings,
	...
	do {
		...
		if ((mask >>= 1) == 0) {  /* Shift mask right one bit. */
			for (i = 0; i < code_buf_ptr; i++)  /* Send at most 8 units of */
				mputc(code_buf[i], outfile);     /* code together */
	...
	if (code_buf_ptr > 1) {		/* Send remaining code. */
		for (i = 0; i < code_buf_ptr; i++) mputc(code_buf[i], outfile);
		codesize += code_buf_ptr;
	}
...
void gbalzssDecode(memstream *infile, memstream *outfile)	/* Just the reverse of Encode(). */
// also fixed to decode in the way bios funktion works
{
	int  i, j, k, r, c, z;
	unsigned int  flags;
	u32 decomp_size;	// iｴ・using decomp_size and cur_size to make sure we dont "decompress" the padding data
	u32 cur_size=0;		// we added to the compressed data to keep it in 4 byte boundaries

	// discard gba header info
	u32 gbaheader;
	unsigned char* tmp = (unsigned char*)&gbaheader;
	for(i=0; i<4; i++) tmp[i] = mgetc(infile);
	...
	for ( ; ; ) {
		...
		if (z == 8) {				// read new block flag
			if ((c = mgetc(infile)) == EOF) break;
			flags = c;
			z = 0;				// reset counter
		}
		if (!(flags&0x80)) {			// flag bit zero => uncompressed
			if ((c = mgetc(infile)) == EOF) break;
			if(cur_size<decomp_size) mputc(c, outfile);  text_buf[r++] = c;  r &= (N - 1); cur_size++;
		} else {
...
// function patched to produce gba bios decompression function processable data
{
	...
	// Bit 4-7   Compressed type (must be 1 for LZ77)
	// Bit 8-31  Size of decompressed data
	u32 gbaheader = 0x10 + (filelength(fileno(infile))<<8);
	unsigned char* tmp = (unsigned char*)&gbaheader;
	//printf("gba header: %x\n", gbaheader );
	for(i=0; i<4; i++) fputc( tmp[i], outfile );
...
void gbalzssDecodeFile(FILE *infile, FILE *outfile)	/* Just the reverse of Encode(). */
// also fixed to decode in the way bios funktion works
{
	int  i, j, k, r, c, z;
	unsigned int  flags;
	...
	u32 gbaheader;
	unsigned char* tmp = (unsigned char*)&gbaheader;
	for(i=0; i<4; i++) tmp[i] = fgetc(infile);
	decomp_size = gbaheader>>8;
	//printf("gba header: %x, decompressed size: %d\n", gbaheader, decomp_size );
	...
	for ( ; ; ) {
		...
		if (!(flags&0x80)) {			// flag bit zero => uncompressed
			if ((c = fgetc(infile)) == EOF) break;
			if(cur_size<decomp_size) fputc(c, outfile);  text_buf[r++] = c;  r &= (N - 1); cur_size++;
		} else {
