// eSTREAM HC-128 by Hongjun Wu
// http://www.ecrypt.eu.org/stream/hcpf.html
// Term http://www.ecrypt.eu.org/stream/phase3ip.html#hc
// The stream ciphers HC-128 and HC-256 are not patented and are royalty free. Anyone can use HC-128 and HC-256 free of
// charge.
...
#define CHAR_BIT          8
#define _lrotr(val, rot)  (((val) << (sizeof(val) * CHAR_BIT - (rot))) | ((val) >> (rot)))
...
void ECRYPT_keysetup(ECRYPT_ctx* ctx, const u8* key, u32 keysize, /* Key size in bits (128+128*i) */
...
void ECRYPT_process_bytes(int action,                                               /* 0 = encrypt; 1 = decrypt; */
                          ECRYPT_ctx* ctx, const u8* input, u8* output, u32 msglen) /* Message length in bytes. */
{
    u32 i, keystream[16];
