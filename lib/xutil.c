...
unsigned long long int read64(const void* p)
{
    ...
    return x[0] | (x[1] << 8) | (x[2] << 16) | (x[3] << 24) |
           ((unsigned long long int)(x[4] | (x[5] << 8) | (x[6] << 16) | (x[7] << 24)) << 32);
}
...
void write64(void* p, const unsigned long long int n)
{
    ...
    x[0] = n & 0xff, x[1] = (n >> 8) & 0xff, x[2] = (n >> 16) & 0xff, x[3] = (n >> 24) & 0xff, x[4] = (n >> 32) & 0xff,
    x[5] = (n >> 40) & 0xff, x[6] = (n >> 48) & 0xff, x[7] = (n >> 56) & 0xff;
}
    ...
    for (; src[i];)
    {
        ...
        else if ((src[i] & 0xe0) == 0xc0)
        {
            ...
            else
            {
                if (len - j < 2)
                    break;
                dst[j++] = ((src[i] & 0x3f) << 6) | ((src[i + 1] & 0x3f));
            }
            i += 2;
        }
