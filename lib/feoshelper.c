...
int open(const char *name, int flags){
	char flag[8];
	if((flags&3)==O_RDONLY)strcpy(flag,"rb");
	else if((flags&3)==O_WRONLY)strcpy(flag,"wb");
	else if((flags&3)==O_RDWR)strcpy(flag,"r+b");
...
int close(int fd){return fclose((FILE*)fd);}
...
int strcasecmp(const char *s1, const char *s2){
	while(*s1 != '\0' && tolower(*s1) == tolower(*s2))s1++,s2++;
