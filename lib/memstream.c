...
{ // do not call mclose if you call mopen using existing memstream.
    if (!s)
        return EOF;
    free(s);
    return 0;
...
int mcopy(memstream* to, const u32 size, memstream* s)
{
    ...
    mread((unsigned char*)(to->p) + mtell(to), _size, s);
    mseek(to, _size, SEEK_CUR);
...
int mseek(memstream* s, const int offset, const int whence)
{
    ...
    {
        case SEEK_SET:
            if (offset < 0 || s->size < offset)
                return EOF;
            s->current = offset;
            return s->current;
        case SEEK_CUR:
            if ((offset < 0 && s->current < -offset) || (offset > 0 && s->size < s->current + offset))
                return EOF;
            s->current += offset;
            return s->current;
        case SEEK_END:
