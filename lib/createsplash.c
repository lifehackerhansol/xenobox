...
static const int DitherTable4Data8[16]={
	0,4,1,5,
	6,2,7,3,
	1,5,0,4,
	7,3,6,2
...
static void *bitmap_create(int width, int height, unsigned int state){return compbuf;}
...
static void bitmap_destroy(void *bitmap){} //using local buffer so doesn't destroy
...
static int imagedecode(char *name, int *height, int dither){
	...
	if(!strcasecmp(name+strlen(name)-4,".bmp")){
		...
		bmp_create(&gif,&vt);
		if(bmp_analyse(&gif, st.st_size, imgbuf)||gif.width!=256||gif.height==0||gif.height>192||bmp_decode(&gif)){bmp_finalise(&gif);return 1;}
		*height=gif.height;
		bmp_finalise(&gif);
		for(y=0;y<*height;y++)
			for(x=0;x<256;x++){
...
static int _createsplash(const char *path, int version){
	...
	if(version!=SPLASH_MOONSHELL1&&version!=SPLASH_MOONSHELL2&&version!=SPLASH_ISAKUREAL&&version!=SPLASH_M3SAKURA)return 1;
	ini_gets("ConvertSetting","SourceFileMask","",wild,768,path);
	if(strlen(wild)<4||(strcasecmp(wild+strlen(wild)-4,".bmp")/*&&strcasecmp(wild+strlen(wild)-4,".ppm")&&strcasecmp(wild+strlen(wild)-4,".png")*/))return 2;
	...
	getcwd(cwd, 768);
	{
		i=strlen(sdir);
		for(;i>0;i--)
			if(sdir[i-1]=='\\'||sdir[i-1]=='/'){sdir[i]=0;chdir(sdir);break;}
	}

	struct dirent *d;
	DIR *dir=opendir("."); //opendir("xxx/") isn't valid.
	if(!dir){chdir(cwd);return 2;}
	for(;d=readdir(dir);)
		if(wildmatch(wild,d->d_name)){
			if(stat(d->d_name,&st)||st.st_size==0||st.st_size>IMGMAX)continue;
			nframe++;
		}
	rewinddir(dir);
	u32 *array=NULL;
	...
	if(version==SPLASH_M3SAKURA){
		if(nframe>126){closedir(dir);chdir(cwd);return 3;} 
		array=(u32*)malloc(4*nframe);
		if(!array){closedir(dir);chdir(cwd);return 2;}
	}else{
		head=(splash_frame*)malloc(16*nframe);
		if(!head){closedir(dir);chdir(cwd);return 2;}
	}
	out=fopen("splash.ani","wb");
	if(!out){free(head);free(array);closedir(dir);chdir(cwd);return 2;}
	...
	for(i=0;d=readdir(dir);){
		if(wildmatch(wild,d->d_name)){
			...
			if(imagedecode(d->d_name,&height,dither)){fclose(out);free(head);free(array);closedir(dir);chdir(cwd);return 20;}
			...
			if(version==SPLASH_ISAKUREAL){
				...
				for(coor=0;coor<256*height;coor++){ //only copy...
					if((coor&0x3f)==0)fwrite(&flags0,1,4,out);
	...
	fclose(out);free(head);free(array);closedir(dir);chdir(cwd);
