...
static int upsmake(const u8 *pfile, const unsigned int sizfile, const u8 *pnewfile, const unsigned int siznewfile, const unsigned int readsize, FILE *ups){
	u8 u8zero=0;
	unsigned int crc=0,_crc;
	fwrite("UPS1",1,4,ups); crc=crc32(crc,(u8*)"UPS1",4);
	u8 *p;
	encnum(buf,sizfile,&p);
	fwrite(buf,1,p-buf,ups); crc=crc32(crc,buf,p-buf);
	...
	for(;i<readsize;i++){
		if(pfile[i]!=pnewfile[i]){
			address=final=i;
			for(;memcmp_fast(pfile+final,pnewfile+final,min(1,readsize-final));final++)i++; //using gap isn't allowed

			encnum(buf,address-oldoff,&p);
			fwrite(buf,1,p-buf,ups); crc=crc32(crc,buf,p-buf);
			for(;address<final;address++){
				u8 x=pfile[address]^pnewfile[address];
				fwrite(&x,1,1,ups); crc=crc32(crc,&x,1);
			}
			fwrite(&u8zero,1,1,ups); crc=crc32(crc,&u8zero,1);
			oldoff=final+1;
		}
	}
	...
	write32(buf,_crc);
	fwrite(buf,1,4,ups); crc=crc32(crc,buf,4);
...
static int upspatch(u8 *pfile, unsigned int sizfile, const u8 *pups, const unsigned int sizups){ //pass pfile=NULL to get sizfile.
	if(sizups<16||memcmp(pups,"UPS1",4))return 2;
	unsigned int crc=0;
	crc=crc32(crc,pups,sizups-4);
	if(crc!=read32(pups+(sizups-4)))return 2;
	...
	while(1){
		if(new==pups+sizups-12)return 0;
		u64 relative=decnum(new,&new);
		if(new+relative+1>pups+sizups-12)return 1;
		if(pfile){
			pfile+=relative;
			for(;*new;new++)*pfile++^=*new;
			pfile++;
		}else{
			for(;*new;new++);
		}
...
static int _upspatch(FILE *file, FILE *ups){
	...
	switch(ret){
		case 0:
			...
			if(crc==read32(pups+(sizups-12)))patchmode=1;
			if(crc==read32(pups+(sizups-8)))patchmode=2;
			if(!patchmode){free(pfile);fprintf(stderr,"incorrect file\n");return 4;}
			...
			if(patchmode==1){
				if(sizex<sizey){
					u8 *tmp=(u8*)malloc(sizey);memset(tmp,0,sizey);
					if(!tmp){free(pups);free(pfile);fprintf(stderr,"cannot allocate memory\n");return 4;}
					memcpy(tmp,pfile,sizex);
					pfile=tmp;
				}
			}else{
				if(sizex>sizey){
					u8 *tmp=(u8*)malloc(sizex);memset(tmp,0,sizex);
					if(!tmp){free(pups);free(pfile);fprintf(stderr,"cannot allocate memory\n");return 4;}
...
int xenoups(const int argc, const char** argv){
	...
	if((argc<2||isatty(fileno(stdin)))&&argc<3){fprintf(stderr,
			"XenoUPS - yet another UPS maker/patcher v2\n"
	...
	if(argc==2){
		...
		if(isatty(fileno(stdout))){
			file=fopen(argv[1],"r+b");if(!file)goto failfile;
			ups=fopen(argv[2],"rb");if(!ups){fclose(file);goto failfile;}
			flag=0x01;
		}else{
		...
		file=fopen(argv[1],"rb");if(!file)goto failfile;
		newfile=fopen(argv[2],"rb");if(!newfile){fclose(file);goto failfile;}
		ups=fopen(argv[3],"wb");if(!ups){fclose(file);fclose(newfile);goto failfile;}
		flag=0x11;
	}
