...
enum PspCtrlButtons{
	...
	PSP_CTRL_DISC = 0x1000000, PSP_CTRL_MS = 0x2000000, PSP_CTRL_SLIDE = 0x20000000,
};

int keyconvertpsp(const int argc, const char **argv){
	if(argc<2){fprintf(stderr,"keyconvertpsp [int|keys]\n");return 1;}
	unsigned int keys=strtoul(argv[1],NULL,0);
	int i=0;
	if(keys){ //int to keys
#define INTTOKEY(KEY,KEY_PRINT) if(keys&PSP_CTRL_##KEY){if(i)putchar('+');i++;printf(#KEY_PRINT);}
...
#define KEYTOINT(KEY_PRINT,KEY) if(strlen(#KEY_PRINT)==i&&!strncasecmp(buttons,#KEY_PRINT,i)){keys|=PSP_CTRL_##KEY;}
