...
int reverseips(const int argc, const char** argv) {
	...
	while(ipsFilePosition < inputsize - 3) { // skip "EOF"
		fprintf(output, "offset 0x%06X", ((ips[ipsFilePosition]) << 16) | ((ips[ipsFilePosition + 1]) << 8) | (ips[ipsFilePosition + 2]));
		...
		if(payloadLength == 0) {
			...
			for (int i = 0; i < realPayloadLength; i++) {
				char offset[10];
				if(i == 0) sprintf(offset, " 0x%02X", ips[ipsFilePosition + 2]);
				else sprintf(offset, ", 0x%02X", ips[ipsFilePosition + 2]);
            ...
            for (int i = 0; i < payloadLength; i++) {
				char offset[10];
				if(i == 0) sprintf(offset, " 0x%02X", ips[ipsFilePosition + i]);
				else sprintf(offset, ", 0x%02X", ips[ipsFilePosition + i]);
				fprintf(output, "%s", offset);
			}
