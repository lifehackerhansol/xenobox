...
static inline int MOD(unsigned int x, int p)
{
    int i = 0;
    for (; i < p; i++)
        x /= 85;
...
static int base85_decode(const char* t)
{
    ...
    {
        for (i = 0; i < len - 1; i++)
            fputc((c >> (8 * (/*len-1*/ 4 - i - 1))) & 0xff, stdout);
    }
    return 0;
}
...
{
    if (argc > 1)
        return base85_decode(t1);
    else
        return base85_encode(t1);
    // return 0;
}
int xenobase85rfc(const int argc, const char** argv)
{
    if (argc > 1)
        return base85_decode(t2);
    else
        return base85_encode(t2);
    // return 0;
}
