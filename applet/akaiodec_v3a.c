...
static void Phase1(const u8 *t){
	int i=0,j=0;
	if(!memcmp(pbuf+0x0c,"####",4)){fprintf(stderr,"Header already decrypted\n");return;}
	for(;i<0x200;i++)
		if(pbuf[i])
...
int akaiodec_v3a(const int argc, const char **argv){
	int i=1;//2;
/*
	//V4: keychanged
	const u8 *t="CuNt";//argv[1];
	//These two keys are top secret! Don't tell this to anyone! Mongai-Fushutsu!
	const u8 *akaiokey_arm9="DoNt hAx";
	const u8 *akaiokey_arm7="My SHitz";
*/

	const u8 *t=(u8*)"FUCK";//argv[1];
	//These two keys are top secret! Don't tell this to anyone! Mongai-Fushutsu!
	const u8 *akaiokey_arm9=(u8*)"\x82\x39\x48\x53\x63\x24\x86\x41";
	const u8 *akaiokey_arm7=(u8*)"\x85\x56\x34\x21\x12\x67\x35\x14";

	FILE *f;
	struct stat st;

	fprintf(stderr,"AKAIO loader decode in C v3a\n");
	if(argc<i+1){fprintf(stderr,"Usage: akaiodec loader...\n");msleep(1000);return -1;} //Currently KEY is "FUCK"
	//len=strlen(t);
	for(;i<argc;i++){
		...
		fstat(fileno(f),&st);
		if(st.st_size<0x200){fclose(f);fprintf(stderr,"Too small\n");continue;}
