...
int getdiscid(const int argc, const char **argv){
	...
	iso_file_stat s;
	if(iso_get_file_info(&s,argv[1],type,"PSP_GAME/PARAM.SFO")){free(WORK_PTR);fprintf(stderr,"iso_get_file_info failed\n");return 3;}
	char *p=malloc(s.size);
	if(!p){free(WORK_PTR);fprintf(stderr,"malloc (for PARAM.SFO) failed\n");return 4;}
	if(iso_read(p,s.size,argv[1],type,"PSP_GAME/PARAM.SFO")<s.size){free(WORK_PTR);fprintf(stderr,"iso_read failed\n");return 5;}
	free(WORK_PTR);
	...
	int i=0;
	for(;i<nlabel;i++){
		if(!strcmp(p+label_offset+read16(p+20+16*i),argc>2?"TITLE":"DISC_ID")){ //seems to be 16bytes long
