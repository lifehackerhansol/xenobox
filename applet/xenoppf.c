...
static int ppfmake(const u8 *pfile, const unsigned int sizfile, const u8 *pnewfile, const unsigned int siznewfile, FILE *ppf){
	fwrite("PPF10\0",1,6,ppf);
	memset(buf,0,50);fwrite(buf,1,50,ppf);
...
static int ppfpatch(u8 *pfile, unsigned int sizfile, const u8 *_pppf, const unsigned int _sizppf){ //pass pfile=NULL to get sizfile.
	if(_sizppf<56||memcmp(_pppf,"PPF",3))return 2;
	char mode=_pppf[3]-'0';
	if(mode<1||3<mode)return 2;
	...
	if(mode==2){
		if(_sizppf<56+4+1024)return 2;
		if(sizfile && sizfile!=read32(_pppf+56))return 2;
		if(memcmp(pfile+0x9320,_pppf+60,1024))return 2;
		pppf+=56+4+1024;
		sizppf-=56+4+1024;
	...
	if(mode==3){
		...
		sizppf-=56+4;
		if(blockcheck){
			if(memcmp(pfile+(imagetype==0?0x9320:0x80A0),_pppf+60,1024))return 2;
			pppf+=1024;
			sizppf-=1024;
		}
		if(!memcmp(pppf+sizppf-4-16,"@END_FILE_ID.DIZ",16)){
	...
	//now, parse body
	for(;;){
		if(pppf==pppfend)return 0;
		u64 offset=0;
		u8 length=0;
		if(mode==1||mode==2){
			if(pppf+5>pppfend)return 1;
			offset=read32(pppf);
			length=pppf[4];
			pppf+=5;
		}
		if(mode==3){
			if(pppf+9>pppfend)return 1;
			offset=read64(pppf);
			length=pppf[8];
			pppf+=9;
		}

		if(pppf+length*(undo?2:1)>pppfend)return 1;
		if(pfile){
			if(undo){
				if(isReverse==-1){
					fprintf(stderr,"reversible: ");
					if(!memcmp(pfile+offset,pppf+length,length)){isReverse=0;fprintf(stderr,"forward patch.\n");}
					if(!memcmp(pfile+offset,pppf,length)){isReverse=1;fprintf(stderr,"reverse patch.\n");}
					if(isReverse==-1){fprintf(stderr,"integrity check failed.\n");return 1;}
				}
				if(isReverse==0){
					if(memcmp(pfile+offset,pppf+length,length)){fprintf(stderr,"integrity check failed.\n");return 1;}
					memcpy(pfile+offset,pppf,length);
				}
				if(isReverse==1){
					if(memcmp(pfile+offset,pppf,length)){fprintf(stderr,"integrity check failed.\n");return 1;}
...
static int _ppfpatch(FILE *file, FILE *ppf){
	...
	if(~ftell(ppf)){
		...
		for(;;){
			...
			if(!tmp){if(pppf)free(pppf);fprintf(stderr,"cannot allocate memory\n");return 4;}
...
int xenoppf(const int argc, const char** argv){
	...
	if((argc<2||isatty(fileno(stdin)))&&argc<3){fprintf(stderr,
			"XenoPPF - yet another ppf maker/patcher v1\n"
	...
	if(argc==2){
		...
		if(isatty(fileno(stdout))){
			file=fopen(argv[1],"r+b");if(!file)goto failfile;
			ppf=fopen(argv[2],"rb");if(!ppf){fclose(file);goto failfile;}
			flag=0x01;
		}else{
		...
		file=fopen(argv[1],"rb");if(!file)goto failfile;
		newfile=fopen(argv[2],"rb");if(!newfile){fclose(file);goto failfile;}
		ppf=fopen(argv[3],"wb");if(!ppf){fclose(file);fclose(newfile);goto failfile;}
		flag=0x11;
	}
