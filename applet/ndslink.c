...
static int ndslink_link(unsigned char* p, const int offset, const int size, const char* target, const char* link)
{
    ...
    if (banneroffset)
    {
        ...
        if (banner[0] != 1)
            banner[0] = 1; // for moonshell2
        if (banner[1] != 0)
            banner[1] = 0; // for moonshell2
    }
    fclose(f);
...
static int recursive(unsigned char* p, const int offset, const int size, char* targetd, char* targetf, char* linkd,
                     char* linkf)
{
    ...
    while (ent = readdir(d))
    {
        ...
        if (!strcmp(targetf, ".") || !strcmp(targetf, ".."))
            continue;
#if defined(WIN32) || (!defined(__GNUC__) && !defined(__clang__))
        ...
        {
            if (strlen(targetf) < 4 || strcmp(targetf + strlen(targetf) - 4, ".nds"))
                continue;
            makedir(linkd);
            printf("Linking %s to %s... %s\n", targetd + 1, linkd,
                   ndslink_link(p, offset, size, targetd, linkd) ? "Failed" : "Done");
        }
    }
    closedir(d);
    return 0;
...
int ndslink(const int argc, const char** argv)
{
    ...
    if (argc < 4)
    {
        puts("ndslink template /target-dir/ link-dir/");
        return -1;
    }
    ...
    {
        free(p);
        puts("template not mshl2wrap link");
        return 1;
    }
