...
int checkumdsize(const int argc, const char **argv){
	...
	for(;i<argc;i++){
		...
		if(buf[0]!=buf[7]||buf[1]!=buf[6]||buf[2]!=buf[5]||buf[3]!=buf[4]){fclose(f);printf("not valid ISO\n");continue;}
		...
		if(sizeFromStat==sizeFromSectors)printf("OK\n");
		if(sizeFromStat<sizeFromSectors)printf("Bad (-%u)\n",sizeFromSectors-sizeFromStat);
		if(sizeFromStat>sizeFromSectors)printf("Over (%u)\n",sizeFromStat-sizeFromSectors);
	}
	return 0;
}
