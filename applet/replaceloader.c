...
static int patch(byte *nds,const int ndslen,const byte *pD,const int loaderlen){
	...
	for(i=0;i<ndslen-loaderlen;i+=4){
		for(j=0;j<M;j++)
			if(!memcmp(nds+i,loadermagic[j],16)&&pD[0]>=nds[i]){pA=nds+i;goto patch;}
	}
	/*if(!pA)*/{printf("not found loader section or loader revision is older\n");return 1;}
...
int replaceloader(const int argc, const char **argv){
	...
	if(!(fl=fopen(argv[1],"rb"))){printf("cannot open %s\n",argv[1]);return 2;}
	fstat(fileno(fl),&stl);
	if(!(pl=malloc(stl.st_size))){fclose(fl);printf("cannot allocate %d bytes for load.bin\n",(int)stl.st_size);return 3;}
	fread(pl,1,stl.st_size,fl);fclose(fl);
	for(i=0;i<M;i++)
		if(!memcmp(pl,loadermagic[i],16))break;
	if(i==M){printf("the loader is invalid\n");return 4;}
	...
	for(i=2;i<argc;i++){
		...
		rewind(f);
		if(!patch(p,st.st_size,pl,stl.st_size))fwrite(p,1,st.st_size,f);
		fclose(f);
	}
	free(pl);return 0;
}
