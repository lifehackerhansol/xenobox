...
int r4crypt(const int argc, const char** argv)
{
    ...
    if (!isatty(fileno(stdin)) && !isatty(fileno(stdout)))
    {
        if (argc == 2)
        {
            ...
            if (sw)
            {
                r4_process(key, sw - 'd', stdin, stdout);
                return 0;
            }
        }
        if (argc == 1)
        {
            int sw = 0;
            if (!strcasecmp(argv[0], "r4dec"))
                sw = 'd';
            if (!strcasecmp(argv[0], "r4ilsdec"))
                sw = 'd';
            if (!strcasecmp(argv[0], "r4enc"))
                sw = 'e';
            if (!strcasecmp(argv[0], "r4ilsenc"))
                sw = 'e';
            if (sw)
            {
                r4_process(key, sw - 'd', stdin, stdout);
