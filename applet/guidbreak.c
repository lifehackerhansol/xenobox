...
int guidbreak(const int argc, const char **argv){
	FILE *f;
	unsigned char *p,*q;
	int size;
	...
	fprintf(stderr,"MoonShell GUID Patch \"breaker\" in C\n\n");
	if(!(f=fopen(argv[1],"rb+"))){fprintf(stderr,"Cannot open %s\n",argv[1]);return 1;}
	size=filelength(fileno(f));
	if(!(p=malloc(size))){fprintf(stderr,"Cannot allocate %d bytes\n",size);return 1;}
	fread(p,1,size,f);
	rewind(f);
...
///
	...
	if(~crcoffset){
		...
		write32(p+crcoffset+8,arm9mem+4);
		flag=1;
...
///
	...
	if(~crcoffset){
		...
		write32(p+crcoffset+8,arm7mem+4);
		flag=1;
