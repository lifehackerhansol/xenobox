...
static int RLE(const u8* p, const unsigned int size, const unsigned int check, unsigned int* length)
{ // if>8bytes, fill operation will be used.
    int i = 0, j;
    if (size < 3 || check < 3)
        return -1;
    for (; i < size - check; i++)
...
// libxenoips
static int ipsmake(const u8* pfile, const unsigned int sizfile, const u8* pnewfile, const unsigned int siznewfile,
                   FILE* ips)
{
    ...
    for (; i < min(sizfile, siznewfile); i++)
    {
        if (pfile[i] != pnewfile[i])
        {
            ...
            // }
            for (; memcmp_fast(pfile + final, pnewfile + final, min(5, min(sizfile, siznewfile) - final)); final++)
                i++;
            ...
            while (size)
            {
                ...
                if (size > 3)
                    rleaddr = RLE(pnewfile + address + patchofs, _size - patchofs, min(size, 7), &rlelength);
                if (rleaddr != -1)
                {
                    if (rleaddr)
                        ipswrite(pnewfile, address + patchofs, rleaddr, ips);
...
static int ipspatch(u8* pfile, unsigned int* sizfile, const u8* pips, const unsigned int sizips)
{ // pass pfile=NULL to get sizfile.
    unsigned int offset = 5, address = 0, size = -1, u;
    if (sizips < 8 || memcmp(pips, "PATCH", 5))
        return 2;
    if (!pfile)
        *sizfile = 0;
    while (1)
    {
        ...
        offset += 3;
        if (address == 0x454f46 && offset == sizips)
            break;
        if (offset + 2 > sizips)
            return 1;
        ...
        if (size)
        {
            if (offset + size > sizips)
                return 1;
            if (!pfile)
            {
                if (*sizfile < address + size)
                    *sizfile = address + size;
                offset += size;
                continue;
            }
            if (address + size > *sizfile)
                return 1;
            fprintf(stderr, "write 0x%06x %dbytes\n", address, size);
        ...
        else
        {
            ...
            if (address + size > *sizfile)
                return 1;
            fprintf(stderr, "fill  0x%06x %dbytes\n", address, size);
            for (u = address; u < address + size; u++)
                pfile[u] = pips[offset];
            offset++;
        }
    }
...
int xenoips(const int argc, const char** argv)
{
    ...
    {
        fprintf(stderr, "XenoIPS - yet another IPS maker/patcher rev2 v2\n"
