...
int fatpatch(const int argc, const char **argv){
  ...
  if(argc<2){fprintf(stderr,"specify path for nds\n");return 1;}
  ext=strlen(argv[1]);
  if(ext<4||argv[1][ext-4]!='.'){fprintf(stderr,"file has to have extention such as .nds or .dat\n");return 1;}
  ext-=3;

  //read
  f=fopen(argv[1],"rb");
  if(!f){fprintf(stderr,"cannot open nds\n");return 2;}
  fseek(f,0,SEEK_END);
  size=ftell(f);
  fseek(f,0,SEEK_SET);
  if(size>0x2000000){fprintf(stderr,"too big\n");fclose(f);return 3;}
  ys=(unsigned char*)malloc(size);
  if(!ys){fprintf(stderr,"cannot alloc memory\n");fclose(f);return 4;}
  fread(ys,1,size,f);
  fclose(f);
  ...
  fprintf(stderr,"Trying ARM...\n");{
    ...
    for(;i<size-100;i+=4){
      if(
        ((
          ys[i+ 3]==0xe3&&ys[i+ 2]==0x50&&ys[i+ 1]==0x00&&ys[i   ]==0x01 && // cmp r0,#0x1 // gcc prefers this
          ys[i+ 7]==0x9a&&ys[i+ 6]==0x00&&ys[i+ 5]==0x00&&ys[i+ 4]==0x02    // bls +0x10 (skip 3)
        )||(
          ys[i+ 3]==0xe3&&ys[i+ 2]==0x50&&ys[i+ 1]==0x00&&ys[i   ]==0x02 && // cmp r0,#0x2 // RVCT prefers this
        ...
        if((ys[i+19]&0xf0)!=0x30&&(ys[i+19]&0xf0)!=0x90)continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=ys[i]==0x01?1:2;
        p=i+19;
      }
    }
if(p){
    fprintf(stderr,"%sPatched 0x%08X [%s]\n",(ys[p]&0xf0)==0x90?"Already ":"",p,flag==1?"CheckDiskNDS":"MoonShell RVCT");
  ...
  fprintf(stderr,"Trying Thumb...\n");{
    ...
    for(;i<size-100;i+=2){
      if(
        ...
        if(ys[i+ 9]!=0xd3&&ys[i+ 9]!=0xd9)continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=0x100;
        p=i+9;
      }
    }
if(p){
  ...
  fprintf(stderr,"Trying libfat (phase2)...\n");{
    ...
    for(;i<size-100;i+=2){
      if(
        ...
        if(ys[i+6]-ys[i+10]!=2||(ys[i+ 7]!=0xd2&&ys[i+ 7]!=0xd8))continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=-1;
        p=i+7;fix=0xd8; // bcs -> bhi
      }
    }
    for(i=0x200;i<size-100;i+=2){
      if(
        ...
        if(ys[i+8]-ys[i+12]!=2||(ys[i+13]!=0xd9&&ys[i+13]!=0xd3))continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=-1;
        p=i+13;fix=0xd3; // bls -> bcc
       }
    }
if(p){
    fprintf(stderr,"I hope libfat doesn't have phase1 issue.\n");
    fprintf(stderr,"%sPatched 0x%08X [%s]\n",ys[p]==fix?"Already ":"",p,p==i+7?"legacy":"1.4.x");
    //if(ys[p]!=0x0a){fprintf(stderr,"incorrect detection (patch error)\n");free(ys);return -1;}
    ys[p]=fix; //bls (return)
}
  }

  if(!flag){fprintf(stderr,"no hits\n");free(ys);return -1;}
  if(flag==-1)goto write;
  _flag=flag;flag=0;
  ...
  if(_flag==1){
    ...
    for(;i<size-100;i+=4){
      if(
        ys[i+ 3]==0xe3&&                ys[i+ 1]==0x00&&ys[i   ]==0x01 && // cmp r?,#0x1 // gcc prefers this
        ...
        if((ys[i+15]&0xf0)!=0x30&&(ys[i+15]&0xf0)!=0x90)continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=1;
        p=i+15;
      }
    }
    for(i=0x200;i<size-100;i+=4){
      if(
        ys[i+ 3]==0xe3&&                ys[i+ 1]==0x00&&ys[i   ]==0x01 && // cmp r?,#0x1 // gcc prefers this
        ys[i+ 7]==0x9a&&ys[i+ 6]==0x00&&ys[i+ 5]==0x00&&ys[i+ 4]==0x04 && // bls +0x18 (skip 5)
        ys[i+11]==0xe5                                                 && // ldr ...
        ys[i+15]==0xe1&&                ys[i+13]==0x00                 && // cmp r?,r?
                        ys[i+26]==0x00&&ys[i+25]==0x00                    // bcc (write)
      ){
        if((ys[i+27]&0xf0)!=0x30&&(ys[i+27]&0xf0)!=0x90)continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=1;
        p=i+27;
      }
    }
if(p){
  ...
  if(_flag==2){
    ...
    for(;i<size-100;i+=4){
      if(
        ys[i+ 3]==0xe3&&                ys[i+ 1]==0x00&&ys[i   ]==0x02 && // cmp r?,#0x2 // RVCT prefers this
        ys[i+ 7]==0x3a&&ys[i+ 6]==0x00&&ys[i+ 5]==0x00&&ys[i+ 4]==0x04 && // bcc +0x18 (skip 5)
        ys[i+11]==0xe5                                                 && // ldr ...
        ys[i+15]==0xe1&&                ys[i+13]==0x00                    // cmp r?,r?
      ){
        if(
    ...
    for(i=0x200;i<size-100;i+=4){
      if(
        ys[i+ 3]==0xe3&&                ys[i+ 1]==0x00&&ys[i   ]==0x02 && // cmp r?,#0x2 // RVCT prefers this
        ys[i+ 7]==0x3a&&ys[i+ 6]==0x00&&ys[i+ 5]==0x00&&ys[i+ 4]==0x03 && // bcc +0x18 (skip 5)
        ys[i+11]==0xe1&&                ys[i+ 9]==0x00                    // cmp r?,r?
      ){
        if(
          ((ys[i+15]&0xf0)!=0x30&&(ys[i+15]&0xf0)!=0x90)||
          ((ys[i+19]&0xf0)!=0x30&&(ys[i+19]&0xf0)!=0x90)||
          ((ys[i+23]&0xf0)!=0x30&&(ys[i+23]&0xf0)!=0x90)
        )continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=2;
...
if(p){
  ...
  if(_flag==0x100){
    ...
    for(;i<size-100;i+=2){
      if(
        ys[i+ 1]==0x2d&&ys[i   ]==0x01 && // cmp r5,#0x1
        ...
        if(ys[i+11]!=0xd2&&ys[i+11]!=0xd8)continue;
        if(flag){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}flag=0x100;
        p=i+11;
      }
    }
