...
//load to buf
  ...
  if(!f1){fprintf(stderr,"cannot fopen %s\n",filename);return 1;}
  fs=filelength(fileno(f1));
  if(fs>0xffffff){fclose(f1);fprintf(stderr,"file too large: %s\n",filename);return 1;}

//save name
  strcpy(bf,filename);
  p=(u8*)strrchr(bf,'.');
  if(!p)p=(u8*)bf+strlen(bf);
  strcpy((char*)p,".bel");
  f2=fopen(bf,"wb");
  if(!f2){fclose(f1);fprintf(stderr,"cannot fopen %s\n",bf);return 1;}
  ...
  p=buf;p1=s1+8;p2=s2;lp=1;co=0;s1l=8;s2l=0;b=0;
...
//compress
	...
	for(i=0;i<fs;i++){
		...
		b=a;
		++co;
        if(s1l>65535){fclose(f2);free(buf);free(s1);free(s2);fprintf(stderr,"Dictionary Buffer Over\n");return 2;}
	}
    *p1=31;++s1l;*p2=b;++s2l; //endmark
    free(buf);
...
//make header
  ...
  strcpy(bf2,filename);
  p=(u8*)strrchr(bf2,'.');
  if(!p)p=(u8*)strrchr(bf2,'/');
  if(!p)p=(u8*)strrchr(bf2,'\\');
  if(p)p++;else p=(u8*)bf2;
...
//load to buf1
  ...
  if(fs1>0x1100000){fclose(f1);fprintf(stderr,"file too large: %s\n",filename);return 1;}
...
//save name
  ...
  for(i=5;i<8;i++)if(buf[i]=='.'||buf[i]=='/'||buf[i]=='\\')buf[i]='_'; //DTV
  if(!buf[5])
    strcpy((char*)p,".ext");
  else
    strncpy((char*)p+1,(char*)buf+5,3);
  f2=fopen(bf,"wb");
  if(!f2){free(buf);fprintf(stderr,"cannot fopen %s\n",bf);return 1;}
...
//decompress to buf2
  p=buf2;i=0;
  while(i<fs2-1){
    lp=*(p1++);l=(i+(lp&31)>fs2-1)?fs2-i-1:lp&31;i+=l;
    for(j=0;j<l;j++){*(p++)=*(p2++);}
    if(lp!=31){
      if(lp<32)l=*(p1++);else l=lp/32;
      a=*(p2-1);i+=l-1;
      for(j=0;j<l-1;j++){*(p++)=a;}
    }
  }
...
int belon(const int argc, const char **argv){
  ...
  switch(argv[1][0]){
    case 'c': return bel_compress(argv[2]);
    case 'd': return bel_extract(argv[2]);
    default: fprintf(stderr,"cmd should be 'c' or 'd'.\n");return -1;
  }
}
