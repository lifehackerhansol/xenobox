...
int xenogbatrim(const int argc, const char **argv){
	...
	for(;i<argc;i++){
		...
		if(s&0xffff){fclose(f);fprintf(stderr,"Not aligned to 0x10000; it seems already trimmed\n");continue;}
		s-=0x10000;
		for(;s;s-=0x10000){
			fseek(f,s,SEEK_SET);
			fread(buf,1,0x10000,f);
			for(j=65535;j>=0;j--)if(buf[s]!=0&&buf[s]!=0xff){s+=j+1;break;}
		}
		if(s){
