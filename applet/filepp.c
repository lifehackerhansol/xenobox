...
static void initialize(){
  ...
  if(!setlocale(LC_ALL,"Japanese")){fprintf(stderr,"Fatal: setlocale(LC_ALL,\"Japanese\") failed\n");exit(-1);}
}
...
static int unfilepp2(FILE *f){
  ...
  size=ftell(f);
  if(fseek(f,size-6,SEEK_SET))return 1;
  if(fread(buf,1,6,f)<6)return 1;
  if(memcmp(buf,"U-Nai",5))return 1;
  if(fseek(f,size-(headersize=10+buf[5]*2),SEEK_SET))return 1;
  if(fread(buf,1,4,f)<4)return 1;
  head=buf[0]+(buf[1]<<8)+(buf[2]<<16)+(buf[3]<<24);
  tail=size-head-headersize;
  memset(wname,0,sizeof(wname));
  if(fread(wname,2,buf[5],f)<buf[5])return 1;
  ucs2tombs((u8*)name,wname);
  name[buf[5]]=0;
  fseek(f,head,SEEK_SET);

  if(unfilepp2(f)){ //stack
    ...
    if(out==stdout) fflush(stdout); else fclose(out);
  }
  out=fopen(name,"wb");
...
static int unfilepp(const char *file){
  ...
  ret=unfilepp2(f);
  fclose(f);
  if(ret){fprintf(stderr,"Error: file is corrupted\n");}
  return ret;
}
...
static int __filepp(const char *core,const int argc,const char **argv){
  ...
  unsigned pos,offset;
  int i=0;
  if(isatty(fileno(stdout))){fprintf(stderr,"Error: output is terminal\n");return 1;}
  if(core[0]==':')
    in1=stdin;
  else if(!(in1=fopen(core,"rb")))return 1;
  ...
  offset=ftell(stdout);
  if(in1!=stdin)fclose(in1);

  for(;i<argc;offset=ftell(stdout),i++){
    if(!(in2=fopen(argv[i],"rb"))){fprintf(stderr,"Warning: cannot open %s\n",argv[i]);continue;}
    ...
    fclose(in2);
    buf[0]=offset&0xff;buf[1]=(offset>>8)&0xff;buf[2]=(offset>>16)&0xff;buf[3]=(offset>>24)&0xff;
...
int filepp(const int argc, const char **argv){
	...
	if(argc==2)return unfilepp(argv[1]);
