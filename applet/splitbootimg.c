...
{
    while (size > 0)
        fread(buf, 1, size < BUFLEN ? size : BUFLEN, in), size -= (size < BUFLEN ? size : BUFLEN);
}

static void DUMP(FILE* in, FILE* out, unsigned int size)
{
    while (size > 0)
        fread(buf, 1, size < BUFLEN ? size : BUFLEN, in), fwrite(buf, 1, size < BUFLEN ? size : BUFLEN, out),
...
int splitbootimg(const int argc, const char** argv)
{
    ...
    {
        fprintf(stderr, "splitbootimg KERNEL RAMDISK [SECOND] <boot.img\n"
    ...
    if (memcmp(buf, BOOT_MAGIC, BOOT_MAGIC_SIZE))
    {
        fprintf(stderr, "BootMagic wrong\n");
        return 1;
    }
    unsigned int ksize = read32(buf + 8), kaddr = read32(buf + 12), rsize = read32(buf + 16), raddr = read32(buf + 20),
                 ssize = read32(buf + 24), saddr = read32(buf + 28);
    ...
    fread(buf, 1, BOOT_NAME_SIZE, stdin);
    buf[BOOT_NAME_SIZE] = 0;
    counter += BOOT_NAME_SIZE;
    fprintf(stderr, "Board name: %s\n", cbuf);
    fread(buf, 1, BOOT_ARGS_SIZE, stdin);
    buf[BOOT_ARGS_SIZE] = 0;
    counter += BOOT_ARGS_SIZE;
    fprintf(stderr, "Comamnd line: %s\n", cbuf);

    int mul = 1;
    unsigned int koffset = pagesize * mul, roffset = align2p(pagesize * mul, koffset + ksize),
                 soffset = align2p(pagesize * mul, roffset + rsize);
