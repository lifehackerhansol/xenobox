...
int google2fa(const int argc, const char** argv)
{
    if (argc < 2)
    {
        fprintf(stderr, "google2fa - Google TwoFactor Authenticator\n"
    ...
    if (argc > 2)
    {
        ...
        int i = 1;
        for (; i < strtol(argv[2], NULL, 0); i++)
            myfgets(secret, 1024, f);
    ...
    {
        fprintf(stderr, "secret_key max 128bytes (might be programmer's fault?\n");
        return 1;
    }
    u8 key[80];
    memstream in, out;
    mopen(secret, strlen(secret), &in), mopen(key, 80, &out), base32_decode(&in, &out);
    ...
    int otp = 0;
    for (;;)
    { // This program is meant to be terminated with Ctrl+C.
        for (;; msleep(200))
        {
            time(&timer);
            pt = localtime(&timer);
            if ((pt->tm_sec % 30 == 0 || timer_old == 0) && timer != timer_old)
            {
                timer_old = timer;
                break;
            }
        ...
        otp = (((hash[offset] & 0x7f) << 24) | (hash[offset + 1] << 16) | (hash[offset + 2] << 8) | hash[offset + 3]) %
              1000000;
