...
static int _decompress(FILE *in, FILE *out){
	...
	for(;i<total_block;i++){
		...
		if(pos>counter)fread(buf+BUFLEN-(pos-counter),1,pos-counter,in);
...
int xenociso(const int argc, const char **argv){
	int mode;
	...
	if(mode=='d'){
		...
		int threshold=100;
		if(argc>4)threshold=strtol(argv[4],NULL,0);
		...
		fprintf(stderr,"compression level = %d ",mode);
...
argerror:
	fprintf(stderr,
		"xenociso level enc dec [threshold(%%,10-100)]\n"
		"xenociso d < enc > dec\n"
		"Compression Level:\n"
		"         1-9: 7zip (fallback to zlib if 7z.dll/so isn't available\n"
		"    10-10000: zopfli (divided by 10; 500 means 50 zopfli numiterations)\n"
	);
	return -1;
}