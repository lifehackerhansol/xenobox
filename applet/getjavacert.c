...
int getjavacert(const int argc, const char **argv){
	if(isatty(fileno(stdin))){
		fprintf(stderr,
			"unzip -p FILE.apk/jar \"META-INF/*.?SA\" | getjavacert\n"
	...
	if(buf[(BUFLEN/2)]!=0x30||buf[(BUFLEN/2)+1]>0x84){fprintf(stderr,"cannot parse header\n");return 1;}
	u32 len=0;
	if(buf[(BUFLEN/2)+1]<0x80)len=buf[(BUFLEN/2)+1];
	u32 i=0,j=0;
	for(;i<buf[(BUFLEN/2)+1]-0x80;i++){
		buf[(BUFLEN/2)+2+i]=fgetc(stdin);len=(len<<8)+buf[(BUFLEN/2)+2+i];
	}
	if(len>BUFLEN/4){fprintf(stderr,"length too large (%d)\n",len);return 1;}
