...
static unsigned int search(unsigned char *mem, unsigned int size,unsigned int base,unsigned char *check,unsigned int lcheck){
...
stdin_end:;
	...
	memset(buf,0,BUFLEN);
	int i=3;for(;i<argc;i++)buf[4*(i-3)]=strtol(argv[i],NULL,0),lcheck+=4;
