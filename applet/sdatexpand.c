...
if(mode<2){
	...
	for(i=0;i<inforec0->nCount;i++){
		...
		if(!getfilesize(((SDATINFOSSEQ*)((u8*)info+inforec0->nEntryOffset[i]))->fileID,&offset,&size)){
			printf("%d %d %d %s\n",((SDATINFOSSEQ*)((u8*)info+inforec0->nEntryOffset[i]))->fileID,sdat_offset[sdat_iter]+offset,size,getfilename(0,i));
			if(mode==1){
	...
	for(i=0;i<inforec2->nCount;i++){
		...
		if(!getfilesize(((SDATINFOSBNK*)((u8*)info+inforec2->nEntryOffset[i]))->fileID,&offset,&size)){
			printf("%d %d %d %s\n",((SDATINFOSBNK*)((u8*)info+inforec2->nEntryOffset[i]))->fileID,sdat_offset[sdat_iter]+offset,size,getfilename(2,i));
			if(mode==1){
	...
	for(i=0;i<inforec3->nCount;i++){
		...
		if(!getfilesize(((SDATINFOSWAR*)((u8*)info+inforec3->nEntryOffset[i]))->fileID,&offset,&size)){
			printf("%d %d %d %s\n",((SDATINFOSWAR*)((u8*)info+inforec3->nEntryOffset[i]))->fileID,sdat_offset[sdat_iter]+offset,size,getfilename(3,i));
			if(mode==1){
	...
	for(i=0;i<inforec7->nCount;i++){
		...
		if(!getfilesize(((SDATINFOSTRM*)((u8*)info+inforec7->nEntryOffset[i]))->fileID,&offset,&size)){
			printf("%d %d %d %s\n",((SDATINFOSTRM*)((u8*)info+inforec7->nEntryOffset[i]))->fileID,sdat_offset[sdat_iter]+offset,size,getfilename(7,i));
			if(mode==1){
	...
	for(i=0;i<inforec0->nCount;i++){
		...
		if(!getfilesize(((SDATINFOSSEQ*)((u8*)info+inforec0->nEntryOffset[i]))->fileID,&offset_sseq,&size_sseq)){ //this sseq entry is valid
...
*/
			...
			if(sbnk<inforec2->nCount && !getfilesize(((SDATINFOSBNK*)((u8*)info+inforec2->nEntryOffset[sbnk]))->fileID,&offset_sbnk,&size_sbnk)){ //refered sbnk entry is valid
				...
				for(j=0;j<4;j++){
					...
					if(swar[j]<inforec3->nCount){
						if(!getfilesize(((SDATINFOSWAR*)((u8*)info+inforec3->nEntryOffset[swar[j]]))->fileID,offset_swar+j,size_swar+j))count++;
					}
				}
				if(count){
					sdat_entry++;
					...
					if(swar[0]!=0xffff)offset_swar[0]+=sdat_offset[sdat_iter];
					fwrite(&offset_swar[0],4,1,ftmp);
					fwrite(&size_swar[0],4,1,ftmp);
					if(swar[1]!=0xffff)offset_swar[1]+=sdat_offset[sdat_iter];
					fwrite(&offset_swar[1],4,1,ftmp);
					fwrite(&size_swar[1],4,1,ftmp);
					if(swar[2]!=0xffff)offset_swar[2]+=sdat_offset[sdat_iter];
					fwrite(&offset_swar[2],4,1,ftmp);
					fwrite(&size_swar[2],4,1,ftmp);
					if(swar[3]!=0xffff)offset_swar[3]+=sdat_offset[sdat_iter];
...
static int ndsEnumFilesDir(char *prefix, u16 dir_id){
	...
	for(;;file_id++){
		...
		char entry_name[128];
		memcpy(entry_name, &fntT[finfo_ofs+1], name_length); entry_name[name_length] = '\0';
		...
		if(entry_type_directory){
			...
			if(size<512){/*fprintf(stderr,"too short\n");*/continue;}
			...
			if(1
				...
				if(
					!memcmp(header->file.type,"SDAT",4)    &&
					header->file.magic==0x0100feff         &&
...
int sdatexpand(const int argc, const char **argv){
	...
	if(argc<3){
		fprintf(stderr,
			"SDATExpand: SDAT parser / NitroMusicPlayer info generater\n"
	...
	mode=-1;
	if(argv[1][0]=='l'||argv[1][0]=='L')mode=0;
	if(argv[1][0]=='e'||argv[1][0]=='E')mode=1;
	if(argv[1][0]=='x'||argv[1][0]=='X')mode=1;
	if(argv[1][0]=='s'||argv[1][0]=='S')mode=2;
	if(mode>2){fprintf(stderr,"cmd = [lexs]\n");return -1;}
	...
	if(mode==2&&argc<4){fprintf(stderr,"specify sps name\n");return -1;}

	fnds=fopen(argv[2],"rb");
	if(!fnds){fprintf(stderr,"fopen failed\n");return -1;}
	...
	if(
		...
		if(fntT_offset+fntT_size<size && fatT_offset+fatT_size<size){
			fntT=(u8*)malloc(fntT_size);fseek(fnds,fntT_offset,SEEK_SET);fread(fntT,1,fntT_size,fnds);
			fatT=(u8*)malloc(fatT_size);fseek(fnds,fatT_offset,SEEK_SET);fread(fatT,1,fatT_size,fnds);
	...
	if(mode==2){
		...
		if(!finfo){fprintf(stderr,"cannot open %s\n",argv[3]);return -1;}
		int i;
		for(i=0;i<4;i++)fputc(0,finfo);
...
#if defined(WIN32) || (!defined(__GNUC__) && !defined(__clang__))
		{
			...
			for(i=0;i<4;i++)fputc(0,finfo); //dummy sseq_data_offset
	...
	if(mode==2){
		fclose(ftmp);
		if(!sdat_entry){fprintf(stderr,"SDAT doesn't have valid entry (perhaps strm)\n");fclose(finfo);remove(argv[3]);remove(TMPFILE);return 1;}
