...
#endif
        for (; (idx = strchrindex((char*)buf, '{', last)) != -1;)
        { // parse header
            if (idx > strlen((char*)buf) - 5)
                break;
            idx2 = strchrindex((char*)buf, '}', idx + 1);
            if (idx2 == -1)
                break;
...
patch:
    ...
    for (; myfgets((char*)buf, BUFLEN, db);)
    {
        if (!*buf)
            continue;
        if (strchr((char*)buf, '['))
            break; // I need insurance if GameListV2Builder.pl fails
        if (strchr((char*)buf, '{'))
            break;
        ...
        if (memcmp(before, openpatch_comp, patchlen))
            differ_before = 1;
        if (memcmp(after, openpatch_comp, patchlen))
            differ_after = 1;
    ...
    if (differ_before)
    {
        ...
        u32 key;
#ifdef LIBVALKYRIA_H
        for (; scanKeys(), !(key = keysDown());)
            swiWaitForVBlank();
#else
        for (; !(key = IPCZ->keysdown);)
            swiWaitForVBlank();
#endif
        if (key & KEY_A)
            ;
...
{
    if (!strcpy_safe(
            buf, findpath(6, (char*[]){ "/", "/_dstwoplug/", "/ismartplug/", "/_iMenu/_ini/", "/_plugin_/", mypath },
                          "GameList.txt")))
    {
        printf("not found GameList.txt\n");
        return -1;
    }
...
int openpatch_single(const int argc, const char** argv)
{
    if (argc < 3)
    {
        printf("openpatch_single patch.txt ROM.nds...\n");
        return -1;
    }
    if (access(argv[1], 0))
    {
        printf("patch.txt isn't accesible\n");
        return -1;
    }
    int i = 2;
    for (; i < argc; i++)
