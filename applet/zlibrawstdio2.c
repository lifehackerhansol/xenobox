...
static int fread2(void* h, char* p, int n)
{
    return fread(p, 1, n, (FILE*)h);
}
static int fwrite2(void* h, const char* p, int n)
{
    return fwrite(p, 1, n, (FILE*)h);
}
...
static int _decompress()
{
    ...
    for (;;)
    {
        ...
        if (z.avail_in == 0)
        {
            ...
            if (z.avail_in == 0)
            {
                fprintf(stderr, "deflated data truncated.\n");
                return 11;
            }
        }
        if (z.avail_out == 0)
