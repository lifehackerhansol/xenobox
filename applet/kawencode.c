...
static int base64_encode(unsigned char k,unsigned char *str,FILE *out){
	...
	for(;I<strlen(str);I++){
		...
		i++;
		b+=8;
		while(b>=6)b-=6,fputc(t[(x>>b)&0x3f],out),j++;
		//if(j==76){fputc('\n',out);j=0;}
	}
	if(b)fputc(t[(x<<(6-b))&0x3f],out),j++;
	i%=3;
	if(i==2)fputs("=",out);
	if(i==1)fputs("==",out);
	//if(j)fputc('\n',out);
...
static int base64_decode(unsigned char k,unsigned char *str,FILE *out){
	...
	for(;i<strlen(str);i++){
		...
		if(p=strchr(t,str[i])){
			x=(x<<6)+(p-t);
			b+=6;
			if(b>=8)b-=8,fputc(((x>>b)&0xff)^k,out);
		}
	}
	while(b>=8)b-=8,fputc(((x>>b)&0xff)^k,out);
	return 0;
}
...
int kawencode(const int argc, const char **argv){
	...
	if(argc>1)for(k=0;i<strlen(argv[1]);i++)k+=argv[1][i];
	if(argc>2)fcrypt=1;
	for(;myfgets(buf,BUFLEN,stdin);){
		unsigned char *p=buf;
		for(;*p==' '||*p=='\t';p++);
		if(!strcmp(p,":crypt")){fcrypt=1;continue;}
		if(!strcmp(p,":endcrypt")){fcrypt=0;continue;}
		if(fcrypt){
			if(argc>1){
