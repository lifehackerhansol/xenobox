...
// .Net TripleDESCryptoServiceProvider compatible routine
...
int ilshield_3des(const int argc, const char **argv){
	if(argc<2||isatty(fileno(stdin))||isatty(fileno(stdout))){fprintf(stderr,"ilshield_3des keyfile/:password [d] < in > out\n");return 1;} 
	u32 key1[32],key2[32],key3[32];
	u8 iv[24],tmp[8];
	int i;
	...
	for(i=0;i<32;i++){
		iv[i%24] = (byte)(iv[i%24]^sha256ctx.buffer.u8[i]);
		iv[(i+16)%24] = (byte)(iv[(i+16)%24]^sha256ctx.buffer.u8[i]); //better randomization
	}
	des_ky(iv,key1);
	des_ky(iv+8,key2);
	des_ky(iv+16,key3); //keybit 128 means key1==key3 i.e. des_ky(iv,key3)
	...
	if(argc>2){ //decrypt
		...
		for(;;){
			...
			if(readlen<0)break;
			if(readlen<8){ //process PKCS5(7) padding
