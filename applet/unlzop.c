...
int lzo1xy_decompress(memstream* mdec, memstream* menc, int lzo1y)
{
    ...
    if (!mdec || !menc || !mavail(mdec) || !mavail(menc))
        return -1;
    c = mgetc(menc);
    if (c > 17)
    {
        if (mcopy(mdec, c - 17, menc) < c - 17)
        {
            error = 1;
            goto done;
        }
    ...
    while (!error)
    {
        ...
        else if (c > 31)
        {
            ...
            if ((arg1 = mgetc(menc)) == EOF || (arg2 = mgetc(menc)) == EOF)
            {
                error = 1;
                break;
            }
        ...
        else if (!state)
        {
            ...
            if (mcopy(mdec, count + 3, menc) < count + 3)
            {
                error = 1;
                break;
            }
        ...
        if (mtell(mdec) < back || back <= 0)
        {
            error = 2;
            break;
        }
        mwrite((unsigned char*)(mdec->p) + mtell(mdec) - back, count + 2, mdec);
        state = count = c & 3;
        if (mcopy(mdec, count, menc) < count)
        {
            error = 1;
            break;
        }
...
int decode_stdin()
{
    ...
    // header
    if (fread(buf, 1, 9, stdin) < 9)
        return 1;
    if (!(buf[0] == 0x89 && buf[1] == 0x4c && buf[2] == 0x5a && buf[3] == 0x4f && buf[4] == 0x00 && buf[5] == 0x0d &&
          buf[6] == 0x0a && buf[7] == 0x1a && buf[8] == 0x0a))
        return 2;

    // need to start hashing...
    r.adler32 = 1;
    r.crc32 = 0;
    if (reader_fread16(&r, &h.ver) < 2)
        return 1;
    if (h.ver < V1)
        return 2;
    if (reader_fread16(&r, &h.libver) < 2)
        return 1;
    if (h.ver >= V2)
    {
        if (reader_fread16(&r, &h.exver) < 2)
            return 1;
        if (h.exver < V1)
            return 2;
    }
    if (reader_fread8(&r, &h.method) < 1)
        return 1;
    if (0x10 <= h.method && h.method <= 0x2f)
        return -1; // nrv is not supported.
    if (h.ver >= V2)
        if (reader_fread8(&r, &h.level) < 1)
            return 1;
    if (reader_fread32(&r, &h.flags) < 4)
        return 1;
    h.os = h.flags >> 24;
    if (h.flags & F_H_FILTER)
    {
        if (reader_fread32(&r, &h.filter) < 4)
            return 1;
        if (h.filter < 17)
            return 2;
    }
    if (reader_fread32(&r, &h.mode) < 4)
        return 1;
    if (h.flags & F_STDIN)
        h.mode = 0;
    if (reader_fread32(&r, &h.mtlow) < 4)
        return 1;
    if (h.ver >= V2)
    {
        if (reader_fread32(&r, &h.mthigh) < 4)
            return 1;
        if (h.ver < 0x1020)
        {
            if (h.mtlow == -1)
    ...
    if (reader_fread8(&r, &l) < 1)
        return 1;
    if (l)
        if (reader_freadstr(&r, (unsigned char*)&h.name, l) < l)
            return 1;
    cs1 = h.flags & F_H_CRC32 ? r.crc32 : r.adler32;
    if (reader_fread32(&r, &cs2) < 4)
        return 1;
    if (cs1 != cs2)
        return 2;
    if (h.flags & F_H_EXTRA_FIELD)
    {
        ...
        r.adler32 = 1;
        r.crc32 = 0;
        if (reader_fread32(&r, &u) < 4)
            return 1;
        for (; u--;)
            if (reader_fread8(&r, NULL) < 1)
                return 1;
        cs1 = h.flags & F_H_CRC32 ? r.crc32 : r.adler32;
        if (reader_fread32(&r, &cs2) < 4)
            return 1;
    ...
    for (;;)
    {
        ...
        if (reader_fread32(&r, &dlen) < 4)
            return 1;
        if (dlen == 0)
            break;
        if (reader_fread32(&r, &clen) < 4)
            return 1;
        if (h.flags & F_ADLER32_D)
            if (reader_fread32(&r, &dadler32) < 4)
                return 1;
        if (h.flags & F_CRC32_D)
            if (reader_fread32(&r, &dcrc32) < 4)
                return 1;
        if (dlen > clen)
        {
            if (h.flags & F_ADLER32_C)
                if (reader_fread32(&r, &cadler32) < 4)
                    return 1;
            if (h.flags & F_CRC32_C)
                if (reader_fread32(&r, &ccrc32) < 4)
                    return 1;
        }
        else
        {
        ...
        r.adler32 = 1;
        r.crc32 = 0;
        if (reader_freadstr(&r, pc, clen) < clen)
        {
            free(pc);
            return 1;
        }
        if (h.flags & F_ADLER32_C && r.adler32 != cadler32)
        {
            free(pc);
            return 2;
        }
        if (h.flags & F_CRC32_C && r.crc32 != ccrc32)
        {
            free(pc);
            return 2;
        }
        if (dlen == clen)
        {
            if (h.flags & F_ADLER32_D && r.adler32 != dadler32)
            {
                free(pc);
                return 2;
            }
            if (h.flags & F_CRC32_D && r.crc32 != dcrc32)
            {
                free(pc);
                return 2;
            }
        ...
        if (h.flags & F_H_FILTER)
        { // process pd with special filter
            pc = pd;
...
int unlzop(const int argc, const char** argv)
{
    ...
    switch (i)
    {
        ...
        case 3:
            ...
            break;
        case -1:
            fprintf(stderr, "NRV is not supported\n");
            break;
        default:
