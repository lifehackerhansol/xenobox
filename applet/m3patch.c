...
static int txt2bin(const char* src, u8* dst)
{
    ...
    if (i < 8 || i & 3)
    {
        printf("m3p line length must be 8,12,16,20,...\n");
        exit(-1);
    }
    return i;
    // puts("");
...
int m3patch(const int argc, const char** argv)
{
    ...
    if (argc < 3)
    {
        printf("m3patch patch.m3p file...\n");
        return 1;
    }

    f = fopen(argv[1], "r");
    if (!f || !myfgets(z, 512, f))
    {
        if (f)
            fclose(f);
    ...
    while (myfgets(z, 512, f))
        checksize[n] = txt2bin(z, check[n]), n++;
