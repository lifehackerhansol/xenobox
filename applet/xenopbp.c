...
static char* filename[8] = { "PARAM.SFO", "ICON0.PNG", "ICON1.PMF", "PIC0.PNG",
...
static void fcopy(FILE* in, FILE* out, unsigned int size)
{
    ...
    for (; size > 0; size -= BUFLEN)
    {
        ...
        if (readlen < (size < BUFLEN ? size : BUFLEN) || size < BUFLEN)
            break;
    }
}
...
int xenopbp(const int argc, const char** argv)
{
    ...
    if (argc == 2)
    {
        ...
        if (fsize < 40 || fread(head, 1, 40, in) < 40 || read32(head + 36) > fsize || head[0] ||
            memcmp(head + 1, "PBP", 3))
        {
            fclose(in);
        ...
        for (i = 0; i < 8; i++)
            fileoffset[i] = read32(head + 8 + 4 * i);
        for (i = 0; i < 7; i++)
            filesize[i] = fileoffset[i + 1] - fileoffset[i];
    ...
    if (argc == 10)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            if (!strcmp(argv[2 + i], "NULL") || !strcmp(argv[2 + i], "null"))
            {
                fprintf(stderr, "[%d] %10d bytes | %s\n", i, 0, argv[2 + i]);
                continue;
            }
        ...
        for (i = 0; i < 8; i++)
            write32(head + 8 + 4 * i, fileoffset[i]);
