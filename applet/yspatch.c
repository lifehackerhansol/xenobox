...
int yspatch(const int argc, const char **argv){
  ...
  if(argc<2){fprintf(stderr,"specify path for ysmenu\n");return 1;}
  ext=strlen(argv[1]);
  if(ext<4||argv[1][ext-4]!='.'){fprintf(stderr,"ysmenu has to have extention such as .nds or .dat\n");return 1;}
  ext-=3;

  //read
  f=fopen(argv[1],"rb");
  if(!f){fprintf(stderr,"cannot open ysmenu\n");return 2;}
  fseek(f,0,SEEK_END);
  size=ftell(f);
  fseek(f,0,SEEK_SET);
  ...
  //patch
  {
    ...
    for(;i<size-100;i+=4){
      if(
        ys[i+ 3]==0xe1&&ys[i+ 2]==0xa0&&ys[i+ 1]==0x00&&ys[i   ]==0x02 && // mov r0,r2
        ...
        /*ys[i+31]==0x0a&&*/ys[i+30]==0x00&&ys[i+29]==0x00&&ys[i+28]==0x14    // beq ... (if not infinite-loop)
      ){
        if(p){fprintf(stderr,"multiple hits (patch error)\n");free(ys);return -1;}
        p=i+31;
      }
    }
    if(!p){fprintf(stderr,"no hits (patch error)\n");free(ys);return -1;}
    if(ys[p]!=0x0a){
      if(ys[p]==0xea){fprintf(stderr,"already patched\n");free(ys);return -1;}
