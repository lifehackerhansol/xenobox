...
static int make(const byte* nds, const int ndslen, const char* p)
{
    ...
    if (!pA)
    { /*fprintf(stderr,"not found DSBooter\n");*/
        return 1;
    }
    ...
    */
    ...
    if (f)
    { // This buffer can be used as ldfBuf in ret_menu_Gen.
...
int m3make(const int argc, const char** argv)
{
...

    {
        ...
        if (i == 0x100)
        {
            fprintf(stderr, "Cannot decode or not DSBooter\n");
            return 3;
        }
