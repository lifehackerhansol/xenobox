...
static int wipsmake(const u8* pfile, const unsigned int sizfile, const u8* pnewfile, const unsigned int siznewfile,
                    FILE* wips)
{
    current_end = 0;
    memset(buf, 0, 0x28);
...
static int wipspatch(u8* pfile, unsigned int sizfile, const u8* _pwips, const unsigned int _sizwips)
{ // pass pfile=NULL to get sizfile.
    if (_sizwips < 0x29 || memcmp(_pwips, "WQSG-PATCH3.0\0\0\0", 16) || read32(_pwips + 0x1c) != 0x28)
        return 2;
    ...
    if (sizfile && sizfile != read64(_pwips + 0x10))
        return 2;
...
static int _wipspatch(FILE* file, FILE* wips)
{
    ...
    switch (ret)
    {
        case 0:
            ...
            if (crc && crc != crc32(0, pfile, sizfile))
            {
                free(pfile);
...
int xenowips(const int argc, const char** argv)
{
    ...
    {
        fprintf(stderr, "XenoWIPS - yet another wips maker/patcher v1\n"
