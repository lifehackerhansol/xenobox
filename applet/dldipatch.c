...
int dldishow(const byte *pD,const int dldilen){
	...
	pDdata=read32(pD+dataStart);pDbssEnd=read32(pD+bssEnd);
	memcpy(id,pD+ioType,4);id[4]=0;
...
#if defined(FEOS) || !(defined(ARM9) || defined(ARM7))
int dldipatch(const int argc, const char **argv){ //for PC main()
	int i,dldisize;
	FILE *f,*fdldi;
	...
	if(!(pdldi=malloc(dldisize)))
		{fclose(fdldi);PRINTF("cannot allocate %d bytes for dldi\n",(int)dldisize);return 3;}
	fread(pdldi,1,dldisize,fdldi);fclose(fdldi);

	for(i=0;i<dldisize-0x80;i+=4){
		if(!memcmp(pdldi+i,dldimagic,12)&&(read32(pdldi+i+dldiVersion)&0xe0f0e0ff)==1)
			{pd=pdldi+i;dldisize-=i;break;}
	}
	if(!pd){PRINTF("the dldi file is invalid\n");return 4;}
	u32 dldilen=*((u32*)(pd+bssStart))-*((u32*)(pd+dataStart)); //DLDITool 0.32.4
	if(argc==2)dldishow(pd,dldilen);
	...
	for(i=2;i<argc;i++){
		...
		PRINTF("Patching %s...\n",name);
		if(!(f=fopen(name,"rb+"))){PRINTF("cannot open %s\n",name);continue;}
		fstat(fileno(f),&st);
		if(!(p=malloc(st.st_size))){PRINTF("cannot allocate %d bytes for %s\n",(int)st.st_size,name);continue;}
		fread(p,1,st.st_size,f);
		rewind(f);
		if(!dldi(p,st.st_size,ignoresize,pd,dldilen))fwrite(p,1,st.st_size,f);
		fclose(f);
		free(p);
	}
	free(pdldi);return 0;
