...
if(nds){
	...
	unsigned int length;
	q=p;
	for(i=0;i<count;q+=length,i++){ //find 0xd0000000
		address=*q++;
		length=*q++;
		if(length&3){fprintf(stderr,"length isn't 4*n\n");return 1;}
		length/=4;
		if(address==0xd0000000){specialpatch=i;}//break;}
	}
	...
	for(i=0;i<count;q+=length,i++){
		...
		if(specialpatch!=-1 && i<=specialpatch){
			fprintf(stderr,"\n");
			if(i==specialpatch)fprintf(stderr,"[above is DSTT specific patch]\n");
			continue;
		}

		unsigned int address_reloc=address;
		if(arm9address<=address&&address<arm9end)address_reloc+=arm9reloc;
		else if(arm7address<=address&&address<arm7end)address_reloc+=arm7reloc;
...
if(nds){
		...
		if(nds)fseek(nds,address_reloc,SEEK_SET);
		//info writer
		for(j=0;j<length;j++){
			if(address_reloc!=address){
...
static int stage1(FILE *extinfo, FILE *nds, unsigned int gamecode, unsigned int CRC32){ //parse extinfo header
	unsigned int count;
	...
	for(;i<count;i++){
		...
		if(ext.gamecode==gamecode&&(ext.crc32==CRC32||!ext.crc32||!CRC32)){ //buggy?
	...
	if(i==count)fprintf(stderr,"not found.\n");
	return 0;
}

int extinfo2binary(const int argc, const char **argv){
	FILE *extinfo=NULL,*nds=NULL;
	if(argc<3){fprintf(stderr,"extinfo2cheat extinfo.dat [game.nds|GAME:aabbccdd]...\n");return 1;}
	if(!(extinfo=fopen(argv[1],"rb"))){fprintf(stderr,"cannot open extinfo\n");return 2;}
	fread(head,1,8,extinfo);
	if(memcmp(head,"ExtInfo",8)){fprintf(stderr,"not extinfo\n");fclose(extinfo);return 3;}
	int i=2;
	for(;i<argc;i++){
		if(argv[i][4]!=':'){
			fprintf(stderr,"%s: ",argv[i]);
			if(!(nds=fopen(argv[i],"r+b"))){fprintf(stderr,"cannot open nds\n");continue;}
