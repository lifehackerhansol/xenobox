...
int decievo(const int argc, const char **argv){
	if(argc<3){fprintf(stderr,"decievo boot.ievo ievo.nds [ievo.head]\n");return 1;}
	FILE *in=fopen(argv[1],"rb");
	if(!in){fprintf(stderr,"fopen %s failed\n",argv[1]);return 2;}
	FILE *out=fopen(argv[2],"wb");
	if(!out){fprintf(stderr,"fopen %s failed\n",argv[2]);return 2;}

	FILE *f_head;
	if(argc>3&&(f_head=fopen(argv[3],"wb"))){ //decrypt first area
		u32 offset;
	...
	if(pad9)fwrite(null256,1,pad9,out);
	...
	if(pad7)fwrite(null256,1,pad7,out);
