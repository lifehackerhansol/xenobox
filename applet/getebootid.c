...
static int get_disc_id(const char* eboot_name)
{
    ...
    if (memcmp(head, "\0PBP", 4))
    { //||read32(head+4)!=0x00010000){ //LittleBigPlanet uses 0x00010001... shouldn't check lol
        sceIoClose(fd);
        goto end;
    }
...
#else
        if (!strcmp(p + label_offset + read16(p + 20 + 16 * i), _argc > 2 ? "TITLE" : "DISC_ID"))
        { // seems to be 16bytes long
#endif
...
int getebootid(const int argc, const char** argv)
{
    if (argc < 2)
    {
        fprintf(stderr, "getebootid EBOOT.PBP [T|P]\n");
        return -1;
    }
    _argc = argc;
    ...
    if (ret)
    {
        fprintf(stderr, ret == 1 ? "parse failed\n" : "no DISC_ID label\n");
        return ret;
    }
