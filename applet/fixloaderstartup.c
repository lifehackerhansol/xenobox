...
int fixloaderstartup(const int argc, const char **argv){
  ...
  if(argc<2){fprintf(stderr,"specify path for homebrew\n");return 1;}
  ext=strlen(argv[1]);
  if(ext<4||argv[1][ext-4]!='.'){fprintf(stderr,"homebrew has to have extention such as .nds or .dat\n");return 1;}
  ext-=3;

  //read
  f=fopen(argv[1],"rb");
  if(!f){fprintf(stderr,"cannot open homebrew\n");return 2;}
  fseek(f,0,SEEK_END);
  size=ftell(f);
  fseek(f,0,SEEK_SET);
  ...
  //patch
  {
    ...
    if(ys[p]!=0x08){
      if(ys[p]==0x00){fprintf(stderr,"already patched\n");free(ys);return -1;}
