...
static int cwreadline(FILE *f, std::string &cmd, std::string &arg1, std::string &arg2){
	...
	*cbuf=0;
	while(!*cbuf){
		line++;
		if(!myfgets(cbuf,1024,f))return -1;
	}
	cmd=strtok(cbuf," \t");
	if(cmd=="_S"||cmd=="_G"||cmd=="_C0"||cmd=="_C1"){
		//cout<<cmd<<endl;
		p=cbuf+strlen(cbuf)+1;
		for(;*p==' '||*p=='\t';p++)if(!*p)return 1;
		arg1=p;
		//cout<<arg1<<endl;
		return 0;
	}else if(cmd=="_L"){
...
cwparse_loop:
	if((r=cwreadline(f,cmd,arg1,arg2))>0)return -1;
	//cout<<cmd<<endl;
	if(r==-1){
		//flush v
		...
		g.second.push_back(
			make_pair(
				make_pair(cheat_enable,cheatname), v
			)
		);

		g.first.first=gameID,g.first.second=gamename;
		db.push_back(g);
		g.second.clear();v.clear();
		return 0; //EOF!
	}else if(r==-2){
		...
		if(!first_db){
			...
			g.first.first=gameID,g.first.second=gamename;
			db.push_back(g);
			g.second.clear();v.clear();cheatname="";first_game=1;
		}
		first_db=0;
		gameID=arg1; //won't fix hyphen... I should care homebrew
		if(cwreadline(f,cmd,arg1,arg2))return -1;
		if(cmd!="_G")return 1;
		gamename=arg1;
	}else if(cmd=="_G"){
		if(!first_db){
		...
		first_game=0;
		cheat_enable=0;
		if(cmd[2]=='1')cheat_enable=1;
		cheatname=arg1;
	}else if(cmd=="_L"){
		if(cheatname=="")return 1;
...
static int cwoutput(FILE *f,cwcheatdb &db){ //output cheat.db
	unsigned int i,j,k;
	for(i=0;i<db.size();i++){
	...
	for(i=0;i<src.size();i++){
		...
		for(l=0;l<sg.second.size();l++){
			...
			for(m=0;m<dg.second.size();m++){
				...
				if(sC.second==dC.second)break; //found cheat
			}
			if(m==dg.second.size())dg.second.push_back(sC);
		}
	}
	return 0;
}
...
extern "C" int main(const int argc, const char **argv){
	...
	if(argc<3){
		fprintf(stderr,
			"mergecwc target.db cheat.db merge.db...\n"
		);
		return 1;
	}
	...
	if(ret=cwparse(f,usr)){fprintf(stderr,"Failed.\n");return ret;}
	fprintf(stderr,"Done.\n");
	fclose(f);
	...
	for(;i<argc;i++){
		...
		if(cwparse(f,_merge)){fprintf(stderr,"Failed.\n");fclose(f);continue;}
		fprintf(stderr,"Done.\n");
		fclose(f);
		cwmerge(usr,_merge);
	}
