...
static int stage2(unsigned int* o, unsigned int* n)
{ // parse usrcheat member
    ...
    for (i = 0; i < ncount;)
    {
        ...
        for (; foldercount; foldercount--)
        {
            ...
            v.clear();
            for (; cheatlen; cheatlen--, n++)
                v.push_back(*n);
            if (s.find(v) != s.end())
                *x = 0x01000000 | (*x & 0x00ffffff), ret++;
...
static int stage1(FILE* _old, FILE* _new)
{ // parse usrcheat header
    unsigned char buf[12];
    fread(buf, 1, 12, _old);
    if (memcmp(buf, "R4 CheatCode", 12))
    {
        fprintf(stderr, "usrcheat_old wrong format\n");
        return 3;
    }
    fread(buf, 1, 12, _new);
    if (memcmp(buf, "R4 CheatCode", 12))
    {
        fprintf(stderr, "usrcheat_new wrong format\n");
        return 3;
    }
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (ocur.gamecode == ncur.gamecode && ocur.crc32 == ncur.crc32)
            {
                // puts("");
                // printf("[%c%c%c%c:%08x]
                // ",ocur.gamecode&0xff,(ocur.gamecode>>8)&0xff,(ocur.gamecode>>16)&0xff,(ocur.gamecode>>24),ocur.crc32);
...
extern "C" int updatecheat(const int argc, const char** argv)
{
    ...
    if (argc != 3)
    {
        fprintf(stderr, "updatecheat usrcheat_old.dat usrcheat_new.dat\n");
        return 1;
    }
