...
typedef std::pair<std::pair<u32,                                // whether cheat is ON
                            std::pair<std::string, std::string> // cheat title/note
                            >,
                  std::vector<u32> // actual cheat
                  >
...
typedef std::pair<std::pair<u32,                                // folder count
                            std::pair<std::string, std::string> // folder title/note
                            >,
                  std::vector<cheat>>
    cheatfolder;

typedef std::pair<std::pair<cheatindex,
                            std::string // game title
                            >,
...
{
    if (!piconv || !strcmp(from_code, "") || !strcmp(to_code, "") || !memcmp(from_code, to_code, 3))
        return p; // don't convert
...
static int parse(FILE* f, usrcheat& usr)
{ // parse usrcheat
    // unsigned char buf[12];
    fread(buf, 1, 12, f);
    if (memcmp(buf, "R4 CheatCode", 12))
    {
        fprintf(stderr, "usrcheat wrong format ");
        return 3;
    }
    fseek(f, 76, SEEK_SET);
    fread(buf, 1, 4, f);
    ...
    {
        from_code = "GBK";
        fprintf(stderr, "Encoding=%s ", from_code);
        if (!strcmp(to_code, ""))
        {
    ...
    {
        from_code = "UTF-8";
        fprintf(stderr, "Encoding=%s ", "UTF8");
        if (!strcmp(to_code, ""))
        {
        ...
        { // ok, let's parse internal structure.
            ...
            for (; i < count;)
            {
                ...
                for (; foldercount; foldercount--)
                {
                    ...
                    for (; cheatlen; cheatlen--, p++)
                        C.second.push_back(*p);
                    i++;
                    p = pnext;
...
static int output(FILE* f, usrcheat& usr)
{ // output usrcheat
    // construct header
    memset(buf, 0, 256);
    ...
    if (!strcmp(to_code, "BIG5" ICONV_ARG))
        memcpy(buf + 76, SIG_BIG5, 4);
    if (!strcmp(to_code, "SJIS" ICONV_ARG))
        memcpy(buf + 76, SIG_SJIS, 4);
    if (!strcmp(to_code, "UTF-8" ICONV_ARG))
        memcpy(buf + 76, SIG_UTF8, 4);
    buf[80] = 1;
    fwrite(buf, 1, 256, f);
...
static int merge(usrcheat& dst, usrcheat& src)
{ // merge DB
    ...
    for (i = 0; i < src.size(); i++)
    {
        ...
        {
            if (dst[j].first.first.gamecode == sg.first.first.gamecode &&
                dst[j].first.first.crc32 == sg.first.first.crc32)
                break;
        }
        if (j == dst.size())
        {
        ...
        for (l = 0; l < sg.second.size(); l++)
        {
            ...
            for (m = 0; m < dg.second.size(); m++)
            {
                ...
                if (sf.first.first == 1)
                {
                    if (df.first.first == 1 && sf.second[0].second == df.second[0].second)
                        break; // found cheat
                }
                else
                {
                    if (sf.first.second.first == df.first.second.first)
                    { // folder name found
                        // merge folder
...
#define ICONV "libiconv"
    ...
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv-2.dll");
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv.dll");
#else
#define ICONV "iconv"
    void* hiconv = LoadLibraryA("libiconv.so.2"); // Generic
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv.so");
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv.so.3"); // FreeBSD
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv.so.5");
    if (!hiconv)
        hiconv = LoadLibraryA("libiconf.so.3");
    if (!hiconv)
        hiconv = LoadLibraryA("libiconf.so");
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv.2.dylib"); // OSX
    if (!hiconv)
        hiconv = LoadLibraryA("libiconv.dylib");
    if (!hiconv)
        hiconv = LoadLibraryA(
            "libc.so.6"); // even if C library, need to make sure to load iconv part via dlopen(), due to license.
    if (!hiconv)
        hiconv = LoadLibraryA("libc.so");
    if (!hiconv)
        hiconv = LoadLibraryA("iconv"); // FeOS(lol?)
#endif
    if (!hiconv)
    {
        fprintf(stderr, "dlopen(libiconv) failed. cannot convert charcode.\n");
    }
    else
    {
    ...
    if (argc < 3)
    {
        fprintf(stderr, "mergecheat [:TARGET_CODE] target.dat usrcheat.dat merge.dat...\n"
                        "TARGET_CODE = GBK / BIG5 / SJIS / UTF8\n");
        return 1;
    }
    ...
    if (argv[i][0] == ':')
    {
        ...
        if (!strcmp(conv, "GBK") || !strcmp(conv, "BIG5") || !strcmp(conv, "SJIS") || !strcmp(conv, "UTF8"))
        {
            ...
            if (!strcmp(conv, "UTF8"))
                strcpy(conv, "UTF-8"); // for some compatibility... UTF8 might not be allowed. Let's use UTF-8 instead.
