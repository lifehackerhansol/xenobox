...
extern "C" int openpatch(const int argc, const char** argv)
{
    ...
    if (argc < 3)
    {
        printf("openpatch patch.txt ROM.nds...\n");
        return 1;
    }
    if (!(patch = fopen(argv[1], "r")))
    {
        printf("cannot open patch\n");
        return 2;
    }
    ...
    while (myfgets((char*)buf, BUFLEN, patch))
    { // parse patch
        ...
        { // parse body
            ...
            if (s.length() & 3)
            {
                printf("patch.txt error: length %d (not 4x)\n", (int)s.length());
                errflag = 1;
            }
            int i = 0;
        ...
        for (; i < (int)vcrc.size(); i++)
        {
            if (errflag)
            {
                printf("warn: skipped %08X\n", vcrc[i]);
                continue;
            }
            if (m.find(vcrc[i]) != m.end())
            {
                printf("duplicate: skipped %08X\n", vcrc[i]);
                continue;
            }
            m[vcrc[i]] = v;
        }
