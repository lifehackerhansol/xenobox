...
int _link(const int argc, const char** argv)
{
    if (argc < 2)
    {
        fprintf(stderr, "tell me xenobox path\n");
        return 1;
    }
    int i = USUAL_INDEX;
    if (!strcmp(argv[0], "link-full") || !strcmp(argv[0], "--link-full"))
        i = TRIVIAL_INDEX;
#if defined(WIN32) || !defined(__GNUC__)
    // int f=1;
    typedef int(__stdcall * CreateHardLinkA_type)(const char*, const char*, void*);
    CreateHardLinkA_type pCreateHardLinkA =
        (CreateHardLinkA_type)GetProcAddress(GetModuleHandleA("kernel32"), "CreateHardLinkA");
    if (!pCreateHardLinkA)
        fprintf(stderr, "link isn't supported, fallback to copy...\n");
    for (; i < appsize; i++)
    {
        ...
        if (pCreateHardLinkA)
        {
            ...
            if (!ret)
            {
                fprintf(stderr, "linking failed, fallback to copy...\n"
                                "if NTFS I hope copying also fails\n");
                pCreateHardLinkA = NULL;
                goto copy;
            }
        }
        else
...
int _install(const int argc, const char** argv)
{
    ...
    if (access(exe, 0))
    {
        fprintf(stderr, "Fatal: cannot detect xenobox's presense.\n"
...
{
    fprintf(stdout, "XenoBox itself is licensed under CC0 (aka Public Domain).\n"
...
int main(const int argc, const char** argv)
{
    ...
    exe = (char*)argv[0] + i + 1;
    for (i = TRIVIAL_INDEX; i < appsize; i++)
        if (!strcasecmp(exe, apps[i].name))
            return apps[i].func(argc, argv);
    if (argc < 2)
        usage();
    for (i = 0; i < appsize; i++)
        if (!strcasecmp(argv[1], apps[i].name))
